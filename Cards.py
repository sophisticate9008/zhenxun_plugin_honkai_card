...
class Cards:
    ...
    def __init__(self, role: "Roles", card_name: str, index: int, level: int):
        ...
        if card_name == "速战速决":
            self.color = color
            self.describe = f"所有卡牌获得迅捷"
            def use(self_):
                role.all_fast_card = True
        if card_name == "垒之护":
            self.color = color
